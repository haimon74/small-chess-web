import { Piece, Position, GameState, PieceType, PieceColor, PIECE_VALUES, BoardSize, GameVariant } from '../types/chess';
...
const initializePawnsChess = (board: (Piece | null)[][], boardSize: BoardSize): void => {
  ...
  // Place king on bottom row (white)
  board[rows - 1][kingCol] = { type: 'king', color: 'white', hasMoved: false };
  // Place king on top row (black)
  board[0][kingCol] = { type: 'king', color: 'black', hasMoved: false };

  // Place pawns on second row (black) and second-to-last row (white)
...
const initializeDianaChess = (board: (Piece | null)[][], boardSize: BoardSize): void => {
  const { rows, cols } = getBoardDimensions(boardSize);
  // Diana Chess: Rook, Bishop, Knight, King, Bishop, Rook
  const backRow: PieceType[] = ['rook', 'bishop', 'knight', 'king', 'bishop', 'rook'];

  // Place black back row (row 0)
  for (let col = 0; col < Math.min(cols, backRow.length); col++) {
    board[0][col] = { type: backRow[col], color: 'black', hasMoved: false };
  }
  // Place white back row (last row)
  for (let col = 0; col < Math.min(cols, backRow.length); col++) {
    board[rows - 1][col] = { type: backRow[col], color: 'white', hasMoved: false };
  }
  // Place pawns
  for (let col = 0; col < cols; col++) {
    board[1][col] = { type: 'pawn', color: 'black', hasMoved: false };
...
const initializeLosAlamosChess = (board: (Piece | null)[][], boardSize: BoardSize): void => {
  const { rows, cols } = getBoardDimensions(boardSize);
  // Los Alamos: Rook, Knight, Queen, King, Knight, Rook
  const backRow: PieceType[] = ['rook', 'knight', 'queen', 'king', 'knight', 'rook'];
...
const initializeMallettChess = (board: (Piece | null)[][], boardSize: BoardSize): void => {
  const { rows, cols } = getBoardDimensions(boardSize);
  // Mallett Chess: White: R, N, Q, K, N, R; Black: R, B, Q, K, B, R
  const blackBackRow: PieceType[] = ['rook', 'bishop', 'queen', 'king', 'bishop', 'rook'];
  const whiteBackRow: PieceType[] = ['rook', 'knight', 'queen', 'king', 'knight', 'rook'];

  // Place black back row (row 0)
  for (let col = 0; col < Math.min(cols, blackBackRow.length); col++) {
    board[0][col] = { type: blackBackRow[col], color: 'black', hasMoved: false };
  }
  // Place white back row (last row)
  for (let col = 0; col < Math.min(cols, whiteBackRow.length); col++) {
    board[rows - 1][col] = { type: whiteBackRow[col], color: 'white', hasMoved: false };
  }
  // Place pawns
  for (let col = 0; col < cols; col++) {
    board[1][col] = { type: 'pawn', color: 'black', hasMoved: false };
...
// Helper to get board dimensions from a game state
const getBoardDimensionsFromState = (gameState: GameState): { rows: number; cols: number } => {
  return {
...
const isValidPosition = (row: number, col: number, gameState?: GameState): boolean => {
  if (gameState) {
    const { rows, cols } = getBoardDimensionsFromState(gameState);
...
const calculatePawnMoves = (position: Position, gameState: GameState, moves: Position[]) => {
  ...
  for (const captureCol of [col - 1, col + 1]) {
    if (isValidPosition(row + direction, captureCol, gameState)) {
...
const calculateKnightMoves = (position: Position, gameState: GameState, moves: Position[]) => {
  ...
  for (const move of knightMoves) {
    if (isValidPosition(move.row, move.col, gameState)) {
...
const calculateBishopMoves = (position: Position, gameState: GameState, moves: Position[]) => {
  ...
  for (const direction of directions) {
    ...
    while (isValidPosition(currentRow, currentCol, gameState)) {
...
const calculateKingMoves = (position: Position, gameState: GameState, moves: Position[]) => {
  ...
  for (let rowOffset = -1; rowOffset <= 1; rowOffset++) {
    for (let colOffset = -1; colOffset <= 1; colOffset++) {
      ...
      if (isValidPosition(newRow, newCol, gameState)) {
